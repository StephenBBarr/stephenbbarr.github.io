...
function createApp(updateContent) {
  ...
  function render(section) {
    const html = `<html><body>${renderPage(section, content)}</body></html>`;
    return parseHTML(html).document.body.firstElementChild;
  }
...
test("text sections have one main heading and no skipped heading levels", () => {
  ...
  for (const name of ["About", "Cv", "Contact"]) {
    ...
    for (let i = 1; i < headings.length; i += 1) {
      ...
      assert(level <= previousLevel + 1, name);
    }
  }
  assert.deepEqual(
...
test("CV preserves all public facts with a useful PDF link near the top", () => {
  ...
  for (const entry of cv.experience.entries) {
    ...
    if (entry.subrole) {
      facts.push(entry.subrole.role, ...(entry.subrole.dates || []));
    }
    for (const achievement of entry.achievements) {
      facts.push(achievement.lead, achievement.body);
    }
  }
  for (const entry of cv.education.entries) {
    facts.push(entry.institution, entry.qualification);
  }

  const page = render("cv");
  for (const fact of facts) {
    assert(page.textContent.includes(fact), fact);
  }
  const pdf = page.querySelector('a[href="/assets/Stephen-Barr-CV.pdf"]');
  assert(pdf);
  assert(pdf.hasAttribute("download"));
  const isNearTop = pdf.parentElement.tagName === "HEADER"
    || pdf.parentElement.parentElement.tagName === "HEADER";
  assert.equal(isNearTop, true);
